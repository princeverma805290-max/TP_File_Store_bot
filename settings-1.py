...
<pre>{client.reply_text}</pre>
    """
    disable_btn_text = '✓ ᴅɪsᴀʙʟᴇ ʙᴛɴ' if client.disable_btn else '✗ ᴅɪsᴀʙʟᴇ ʙᴛɴ'
    reply_markup = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(disable_btn_text, 'toggle_disable_btn')],
...
async def toggle_disable_btn(client, query):
    client.disable_btn = False if client.disable_btn else True
    status = "✓ ᴇɴᴀʙʟᴇᴅ" if client.disable_btn else "✗ ᴅɪsᴀʙʟᴇᴅ"
    await query.answer(f"ᴅɪsᴀʙʟᴇ ʙᴜᴛᴛᴏɴ: {status}", show_alert=True)
